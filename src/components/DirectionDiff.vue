...
<script>
  import {month, sensor, factory, threshold, diffChart, sctDataToken, windToken, selectedHour, selectedChemical} from '../vuex/getters'
  ...
  export default {
    vuex: {
      getters: {month, sensor, factory, threshold, diffChart, sctDataToken, windToken, selectedHour, selectedChemical},
      actions: {addDiffChart, switchFactory}
    },
    watch: {
      sctDataToken () {
        if (this.sctDataToken) allData = storage.get(this.sctDataToken)
      },
      selectedHour () {
        Object.keys(this.diffCharts).forEach((chart) => {
          console.log('ddddddddddddddddddddddddddddddddddddddddddddddd', chart, this.selectedHour)
...
//      },
      ...
      selectedChemical () {
        // console.log('direction diff vue selectedChemical')
        this.diffChart.forEach((chart, index) => {
          let selector = '#DirectionDiff-' + chart.sensor + chart.factory
          if (this.diffCharts[selector]) this.diffCharts[selector].clearCurrent()
          this.calcAndDrawDiff(chart, selector)
        })
      },
      threshold () {
        // console.log('direction diff vue threshold')
        this.diffChart.forEach((chart, index) => {
          let selector = '#DirectionDiff-' + chart.sensor + chart.factory
          if (this.diffCharts[selector]) this.diffCharts[selector].clearCurrent()
          this.calcAndDrawDiff(chart, selector)
    ...
    methods: {
      ...
      calcAndDrawDiff (ch, selector) {
        // console.log('cccccccccccccccccccccc calcAndDrawDiff', ch, selector)
        if (!windData && this.windToken) windData = storage.get(this.windToken)
        if (!allData && this.sctDataToken) allData = storage.get(this.sctDataToken)
        let {factory, sensor} = ch
        let month = this.month
        // let chemical = this.selectedChemical
        // 绘图数据是data
        let data = windData
        // let readingData = allData
...
// console.log('here aaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbaaaaaaaaaaaaaaaaaaa')
        let thresholdData = null
        // // thresholdData = this.processThresholdData(readingData, month, chemical)
// console.log('here aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        ...
        this.$nextTick(() => {
          ...
          // console.log('aaaaaaaaaaaaaaaaaaaaaa', month, thresholdData, sensor, this.threshold)
          this.diffCharts[selector].draw(chartData, month, thresholdData, sensor)
        })
      },
      // 风向差异图数据处理
      ...
      processThresholdData (readingData, month, chemical) {
        if (!chemical || !readingData || !month) return null
        // console.log('reading data rrrrrrrrrrrrrrrrrrrrrrrrrrrr', readingData, chemical)
        let resultData = []
        for (let sensorIndex = 1; sensorIndex <= 9; sensorIndex++) {
          let sensorName = 'S' + sensorIndex
          let tmpData = readingData[sensorName][chemical]
          // console.log('ttttttttttt', tmpData)
          Object.keys(tmpData).forEach((d) => {
            // console.log('direction diff vue process threshold data', d)
            let m = new Date(d).getMonth() + 1
            // console.log('ddddddddddddd', d, m, month)
            // let h = new Date(d).getHours()
            if (m === month && tmpData[d] >= this.threshold[chemical]) {
              resultData.push({
                time: d,
                sensor: sensorName
              })
